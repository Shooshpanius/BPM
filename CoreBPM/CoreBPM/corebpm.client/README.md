# Core BPM — Клиентская часть (Frontend)
...
- Запущенный бэкенд (`CoreBPM.Server`) — для работы API-прокси

### Установка зависимостей

```bash
cd CoreBPM/CoreBPM/corebpm.client
npm install
```
...
```bash
# Сборка образа вручную (из каталога corebpm.client/)
docker build -t corebpm-client .

# Запуск контейнера
docker run -p 80:80 corebpm-client
```
...
- [Бэкенд (CoreBPM.Server)](../CoreBPM.Server/README.md)
